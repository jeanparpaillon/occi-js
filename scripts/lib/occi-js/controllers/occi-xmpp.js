...
app.controller('xmppCtrl',
	       ['$scope', 'xmpp', 'xmppSession', 'myService',
		function($scope, xmpp, xmppSession, myService) {
			...
			myService.getDetails().getCollections(location).then(
			    function(result) {
			...
			myService.getDetails().getResourceLocations(resource).then(
			    function(result) {	
